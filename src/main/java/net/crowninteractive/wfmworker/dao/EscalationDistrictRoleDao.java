...
public class EscalationDistrictRoleDao extends AbstractDao<Integer, EscalationSettings> {
    public List<EscalationDistrictRole> fetchDistrictRoleEmails(String businessUnit, String roles) {
        ...
        List<EscalationDistrictRole> options = getEntityManager().createNativeQuery(sql.toString(),
                                                   EscalationDistrictRole.class).setParameter(1,
                                                       findByName(businessUnit).getId()).getResultList();
    ...
    public District findByName(String name) {
        ...
        List<District> options = getEntityManager().createNativeQuery(query, District.class).setParameter(1,
                                     name).getResultList();
