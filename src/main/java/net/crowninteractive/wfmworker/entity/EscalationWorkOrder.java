 ...
 */
@Entity
@Table(name = "escalation_work_order", catalog = "wfm_new", schema = "")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "EscalationWorkOrder.findAll", query = "SELECT e FROM EscalationWorkOrder e"),
    @NamedQuery(name = "EscalationWorkOrder.findById", query = "SELECT e FROM EscalationWorkOrder e WHERE e.id = :id"),
    @NamedQuery(name = "EscalationWorkOrder.findByToken", query = "SELECT e FROM EscalationWorkOrder e WHERE e.token = :token"),
    @NamedQuery(name = "EscalationWorkOrder.findByOwnerId", query = "SELECT e FROM EscalationWorkOrder e WHERE e.ownerId = :ownerId"),
    @NamedQuery(name = "EscalationWorkOrder.findByStatusId", query = "SELECT e FROM EscalationWorkOrder e WHERE e.statusId = :statusId"),
    @NamedQuery(name = "EscalationWorkOrder.findByStatusUpdatedTime", query = "SELECT e FROM EscalationWorkOrder e WHERE e.statusUpdatedTime = :statusUpdatedTime"),
    @NamedQuery(name = "EscalationWorkOrder.findByTimeToEscalate", query = "SELECT e FROM EscalationWorkOrder e WHERE e.timeToEscalate = :timeToEscalate"),
    @NamedQuery(name = "EscalationWorkOrder.findByTimeEscalated", query = "SELECT e FROM EscalationWorkOrder e WHERE e.timeEscalated = :timeEscalated"),
    @NamedQuery(name = "EscalationWorkOrder.findByEscalatedEmails", query = "SELECT e FROM EscalationWorkOrder e WHERE e.escalatedEmails = :escalatedEmails"),
    @NamedQuery(name = "EscalationWorkOrder.findByTs", query = "SELECT e FROM EscalationWorkOrder e WHERE e.ts = :ts")})
public class EscalationWorkOrder implements Serializable {
    ...
    @Basic(optional = false)
    @Column(name = "time_to_escalate", nullable = false)
    @Temporal(TemporalType.TIMESTAMP)
    private Date timeToEscalate;
    @Column(name = "time_escalated")
    @Temporal(TemporalType.TIMESTAMP)
    private Date timeEscalated;
    @Column(name = "escalated_emails", length = 2000)
    private String escalatedEmails;
    @Column(name = "ts")
    @Temporal(TemporalType.TIMESTAMP)
    private Date ts;
    @JoinColumn(name = "escalation_setting_id", referencedColumnName = "id", nullable = false)
...
//"insert into escalation_work_order (token,owner_id,work_order_id,escalation_setting_id,escalated_emails, time_to_escalate, status_id) values (?,?,?,?,?,?,?)",RandomStringUtils.randomAlphanumeric(20),i.getOwnerId(),i.getId(), es.getId(), emails,dt, es.getStatusId() == null ? 0 : es.getStatusId() );
    ...
    public EscalationWorkOrder(int ownerId, WorkOrderStatus statusId, Date timeToEscalate, String escalatedEmails, EscalationSettings escalationSettingId, WorkOrder workOrderId) {
        this.ownerId = ownerId;
        if(statusId != null){
