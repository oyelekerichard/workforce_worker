...
public class Worker extends TimerTask{
    ...
    public void run() {
        try {
            ...
            for (EscalationWorkOrder ewo : lewo) {
                String emailtitle = String.format("WORK ORDER ESCALATION!!! (#00%s) - %s", ewo.getWorkOrderId().getTicketId(), ewo.getEscalationSettingId().getLabel());
                String recipients = ewo.getEscalatedEmails();
                String body = format(FileUtils.readFileToString(new File("/var/files/wfm/request.htm")), String.valueOf(ewo.getWorkOrderId().getTicketId()), ewo.getWorkOrderId().getSummary(), ewo.getWorkOrderId().getQueueId().getName(), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(ewo.getWorkOrderId().getCreateTime()), ewo.getWorkOrderId().getCurrentStatus(), ewo.getWorkOrderId().getDescription());
                sendNoReplyEmail("a12wq_minions", emailtitle, recipients, body);
                System.out.println("Escalation email sent for work order #"+ewo.getWorkOrderId().getTicketId()+"  - PRIORITY: "+ewo.getEscalationSettingId().getPriority());
                
                ewo.setTimeEscalated(new Date());
                escalationWorkOrderDao.edit(ewo);
                //raise  escalation level
                EscalationSettings es2 = escalationSettingsDao.getNextEscalationLevel(ewo.getEscalationSettingId().getStatusId(), ewo.getEscalationSettingId().getQueueTypeId().getId(), ewo.getEscalationSettingId().getPriority()+1);
                if(es2 != null){
                    ...
                    EscalationWorkOrder ew = new EscalationWorkOrder(i.getOwnerId(), es2.getStatusId(), cal.getTime(), emailRecipients(ewo.getWorkOrderId(), es2), es2, i);
    ...
    private String format(String filedata, String ticketId, String summary, String queueName, String simpleDateTime, String currentStatus, String description) {
        return filedata.replace("$ticketid", ticketId).replace("$summary", summary).replace("$queue", queueName).replace("$time", simpleDateTime).replace("$status", currentStatus).replace("$desc", description);
    }
    
    public void sendNoReplyEmail(@WebParam(name = "qw") String emailkey,@WebParam(name = "subject") String subject, @WebParam(name = "to") String to, @WebParam(name="msg") String msg, @WebParam(name="filename") String filename ) {
    ...
    public void sendNoReplyEmail(@WebParam(name = "qw") String emailkey,@WebParam(name = "subject") String subject, @WebParam(name = "to") String to, @WebParam(name="msg") String msg ) {
